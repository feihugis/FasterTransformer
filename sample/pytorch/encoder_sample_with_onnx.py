...
def main():
    ...
    export_onnx_hf = True
    onnx_model_path = '/tmp/hf_encoder.onnx'
    if export_onnx_hf:
        torch.onnx.export(
            hf_encoder,
            (inp, mask),
    ...
    os.environ['KMP_AFFINITY'] = "respect,none"
    os.environ['OMP_MAX_ACTIVE_LEVELS'] = "1"

    so.graph_optimization_level = onnxruntime.GraphOptimizationLevel(99)
    onnx_encoder = onnxruntime.InferenceSession(onnx_model_path, sess_options=so)
    onnx_encoder.set_providers(['CUDAExecutionProvider'])
    onnx_inputs = {'inp': inp.cpu().numpy(), 'mask': mask.cpu().numpy()}
    ...
    with torch.no_grad():
        ...
        print("====== hf_output: \n", hf_output, hf_output.size())

        onnx_output = onnx_encoder.run(['output'], onnx_inputs)[0] * output_mask.cpu().numpy()
        print("onnx output:\n", onnx_output, "\n")

        ft_output = custom_encoder(inp, mask, mem_seq_lens)[0] * output_mask
        print("====== ft_output: \n", ft_output, ft_output.size())

        eff_ft_output = eff_custom_encoder(inp, mask, mem_seq_lens)[0] * output_mask
        print("====== eff_ft_output: \n", eff_ft_output, eff_ft_output.size())
        ...
        diff = torch.abs(hf_output - torch.tensor(onnx_output).cuda())
        print('ONNX Mean diff: {}'.format(torch.mean(diff)))
        print('ONNX Max diff:  {}'.format(torch.max(diff)))
        print('ONNX Min diff:  {}'.format(torch.min(diff)))
        ...
        if args.time:
            ...
            # nvtx.range_push("onnx")
            for i in range(iterations):
                # nvtx.range_push("onnx"+str(i))
                torch.cuda.synchronize()
                onnx_output = onnx_encoder.run(['output'], onnx_inputs)
                torch.cuda.synchronize()
                # nvtx.range_pop()
            # nvtx.range_pop()
            ...
            print("[INFO]                  ONNX time costs: {:.2f} ms".format(t4*1000/iterations))
