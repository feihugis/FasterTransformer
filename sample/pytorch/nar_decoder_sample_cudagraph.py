...
def run():
    ...
    path = '/datadrive/fhu/github/dlis-nar/model_checkpoint/checkpoint_best.pt'
    is_fp16 = False
    ...
    with torch.no_grad():
        ...
        input_seq_len = 6
        for input_seq_len in range(16, 17, 1):
            ...
            t1 = time.time()
            for  _ in range(100):
                # torch.cuda.nvtx.range_push("FT Decoder")
                ft_output, self_cache, mem_cache = custom_decoder(inp, mem, input_seq_len, mem_seq_len, self_cache, mem_cache, 1)
                # torch.cuda.nvtx.range_pop()
            torch.cuda.synchronize()
            t2 = time.time()
            print(f"FT time: {(t2 - t1)/100}")
            ...
            t5 = time.time()
            for _ in range(100):
                # torch.cuda.nvtx.range_push("FT Decoder CUDA_GRAPH")
                g.replay()
                # torch.cuda.nvtx.range_pop()
            torch.cuda.synchronize()
            t6 = time.time()
            print(f"FT replay time: {(t6 - t5)/100} \n")
