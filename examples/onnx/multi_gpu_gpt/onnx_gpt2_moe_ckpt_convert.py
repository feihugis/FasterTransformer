...
def split_and_convert_process(i, saved_dir, factor, key, args, val):
    ...
    elif key.find("mlp.moe.experts") != -1:
        split_vals = np.split(val, factor, axis=-1)
        for j in range(factor):
            if key.find("dense_4h_to_h.bias") != -1:
                saved_path = saved_dir + "/model." + key + ".bin"
            else:
                saved_path = saved_dir + "/model." + key + ".%d.bin" % (i * factor + j)
            split_vals[j].tofile(saved_path)
...
def split_and_convert(args):
    ...
    onnx_model_name_pattern = [
        ...
        "mlp.c_proj.bias",
        "mlp.c_proj.weight",
        "mlp.moe_experts.bias1",
        "mlp.moe_experts.weight1",
        "mlp.moe_experts.bias2",
        "mlp.moe_experts.weight2"
    ]
    ...
    ft_model_name_pattern = [
        ...
        "mlp.moe.experts.dense_h_to_4h.bias",
        "mlp.moe.experts.dense_h_to_4h.weight",
        "mlp.moe.experts.dense_4h_to_h.bias",
        "mlp.moe.experts.dense_4h_to_h.weight",
    ]
    
    proccess_list = []
    sub_graphs = []
    for g in [model.graph, model.graph.node[0].attribute[5].g, model.graph.node[0].attribute[6].g]:
        for t in g.initializer:
            print(t.name)
            if t.name.find("weight") == -1 and t.name.find("bias") == -1:
                continue
            if t.name == 'd_transformer.wpe.weight':
                numpy_helper.to_array(t).astype(np.float32).tofile(saved_dir + "model.wpe.bin")
            elif t.name == 's_d_transformer.wte.weight':
                numpy_helper.to_array(t).astype(np.float32).tofile(saved_dir + "model.wte.bin")
            elif t.name == 'd_transformer.ln_f.bias':
                numpy_helper.to_array(t).astype(np.float32).tofile(saved_dir + "model.final_layernorm.bias.bin")
            elif t.name == 'd_transformer.ln_f.weight':
                ...
                for i in range(len(onnx_model_name_pattern)):
                    if t.name.find(onnx_model_name_pattern[i]) != -1:
                        ...
                        # some weights from `decoder` graph are as same as those from `init_decoder`
                        if new_name.find("d_transformer") != -1:
                            new_name = new_name.replace("d_transformer.", "")
                        if new_name.find("e_transformer") != -1:
                            new_name = new_name.replace("e_transformer.", "")
                        if new_name.find("s_layers") != -1:
                            new_name = new_name.replace("s_layers", "layers")
