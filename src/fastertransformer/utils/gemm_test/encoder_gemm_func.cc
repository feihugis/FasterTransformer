    ...
    int batch_size, int seq_len, int head_num, int size_per_head, void* buffer_in, bool isAppend)
{
    ...
    std::vector<int> M; //[gemm_num];
    std::vector<int> N; //[gemm_num];
    std::vector<int> K; //[gemm_num];
    std::vector<int> batchCount; //[gemm_num] = {1, 1, 1, 1, 1, 1};
    std::vector<int> type;
    std::vector<std::string> mess; //[gemm_num][256];
    std::vector<float> exec_times; //[gemm_num];

    // gemm1
    M.push_back(batch_size * seq_len);
    K.push_back(head_num * size_per_head);
    N.push_back(head_num * size_per_head);
    batchCount.push_back(1);
    type.push_back(0);
    mess.push_back("EFT from_tensor * weightQ/K/V, attr * output_kernel");

    for (int num_tokens = 1; num_tokens < batch_size * seq_len; ++num_tokens) {
        M.push_back(num_tokens);
        N.push_back(head_num * size_per_head);
        K.push_back(head_num * size_per_head);
        batchCount.push_back(1);
        type.push_back(0);
    ...
    M.push_back(batch_size * seq_len);
    K.push_back(head_num * size_per_head);
    N.push_back(4 * head_num * size_per_head);
    batchCount.push_back(1);
    type.push_back(1);
    mess.push_back("EFT attr_output * inter_kernel");

    // gemm3
    M.push_back(batch_size * seq_len); //M[0];
    K.push_back(4 * head_num * size_per_head); //4 * K[0];
    N.push_back(head_num * size_per_head); //N[0];
    batchCount.push_back(1);
    type.push_back(2);
    mess.push_back("EFT inter_matmul * output_kernel");
    
    // gemm4
    M.push_back(seq_len);
    N.push_back(seq_len);
    K.push_back(size_per_head);
    batchCount.push_back(batch_size * head_num);
    type.push_back(3);
    mess.push_back("EFT attention batched Gemm1");
    
    // gemm5
    M.push_back(seq_len);
    N.push_back(size_per_head);
    K.push_back(seq_len);
    batchCount.push_back(batch_size * head_num);
    type.push_back(4);
    mess.push_back("EFT attention batched Gemm2");
    
    // gemm6
    M.push_back(batch_size * seq_len);
    N.push_back(head_num * size_per_head);
    K.push_back(head_num * size_per_head);
    batchCount.push_back(3);
    type.push_back(5);
    mess.push_back("EFT from_tensor * weight_QKV in BatchGemm");
    ...
    const int gemm_num = M.size();
    exec_times.resize(gemm_num);
    ...
    printf("***EFT Encoder Gemm Testing Begin***\n");
    printf("***EFT Cublas Gemm Testing Begin***\n");
    if (line_count == 0) {
        fprintf(fd,
                "batch_size, seq_len, head_num, size_per_head dataType ### batchCount, n, m, k, algoId, "
                "customOption, tile, numSplitsK, swizzle, reductionScheme, workspaceSize, stages, exec_time\n");
    ...
    for (int i = 0; i < M.size(); ++i) {
        ...
        int m = M[i], n = N[i], k = K[i], t = type[i];
        printf("\n-----------------------------\n");
        printf("EFT GEMM test %d: [M: %d, K: %d, N: %d] %s\n", i, m, k, n, mess[i]);
        T* d_A = (T*)buffer;
        T* d_B = d_A + m * k * batchCount[i];
        T* d_C = d_B + k * n * batchCount[i];
        ...
        printf("EFT fast_algo %d costs %.3f ms\n", fast_algo, exec_time);

        // for fp16 and bf16, we compare cublasLt
        if (t < 3 && data_type != FLOAT_DATATYPE) {
            printf("***EFT cublasLt Gemm Testing Beign***\n");
            // Let try a fixed number of combinations
            int ALGO_COMBINATIONS = 5000;
            customMatmulPerf_t perfResults[ALGO_COMBINATIONS];
            LtHgemmCustomFind<T, scaleT>(ltHandle,
            ...
            printf("***EFT cublasLt Gemm Testing End***\n");
        }
        else {
            fprintf(fd,
                    "%d %d %d %d %d ### %d %d %d %d %d -1 -1 -1 -1 -1 -1 -1 %f\n",
    ...
    printf("***EFT cublas Gemm Testing End***\n\n");
    fclose(fd);
    printf("***EFT Encoder Gemm Testing End***\n");
...
template void generate_effective_encoder_gemm_config<float>(
    int batch_size, int seq_len, int head_num, int size_per_head, void* buffer, bool isAppend);
template void generate_effective_encoder_gemm_config<half>(
    int batch_size, int seq_len, int head_num, int size_per_head, void* buffer, bool isAppend);
#ifdef ENABLE_BF16
template void generate_effective_encoder_gemm_config<__nv_bfloat16>(
    int batch_size, int seq_len, int head_num, int size_per_head, void* buffer, bool isAppend);
#endif
